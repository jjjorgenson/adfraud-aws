...
def extract_features(event: Dict[str, Any], context_data: Dict[str, Any] = None) -> List[float]:
    ...
    ip_is_proxy = is_proxy_ip(ip_address)
    ip_is_business = is_business_ip(ip_address)
    
    # Competitor detection
    competitor_ips = event.get('competitor_ips', context_data.get('competitor_ips', []))
    ...
    engagement_score = calculate_engagement_score(click_to_view_time_ms, time_to_conversion_sec if time_to_conversion_sec else None)
