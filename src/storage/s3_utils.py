...
def store_raw_event(bucket_name: str, event: Dict[str, Any], event_id: str, timestamp: datetime) -> str:
    """
    ...
    s3_client.put_object(
        ...
        Metadata={"event_id": event_id, "event_type": event.get("event_type", "unknown"), "timestamp": timestamp.isoformat()},
    )
...
def store_training_data(bucket_name: str, features: Any, labels: Any, version: str = "v1.0") -> Dict[str, str]:
    """
    ...
    s3_client.put_object(Bucket=bucket_name, Key=features_key, Body=json.dumps(features), ContentType="application/json")

    s3_client.put_object(Bucket=bucket_name, Key=labels_key, Body=json.dumps(labels), ContentType="application/json")

    return {"features_key": features_key, "labels_key": labels_key}


def list_events_by_date_range(bucket_name: str, start_date: datetime, end_date: datetime) -> List[str]:
    """
...
def store_model_artifact(bucket_name: str, model_data: bytes, model_version: str) -> str:
    """
    ...
    s3_client.put_object(Bucket=bucket_name, Key=s3_key, Body=model_data, ContentType="application/gzip")
